		...
		<div class="card">
			<div class="card-header with-border">
				<button type="button" class="btn btn-primary" id="registBtn"
					onclick="OpenWindow('registForm.do','공지등록',800,700);">공지등록</button>
				<div id="keyword" class="card-tools" style="width: 540px;">
					<div class="input-group row">
						<select class="form-control col-md-3" name="perPageNum"
							...
							id="searchType">
							<option value="tcw" ${param.searchType eq 'tcw' ? "selected":"" }>전
								체</option>
							<option value="t" ${param.searchType eq 't' ? "selected":"" }>제
								목</option>
							<option value="w" ${param.searchType eq 'w' ? "selected":"" }>작성자</option>
							<option value="c" ${param.searchType eq 'c' ? "selected":"" }>내
								용</option>
					...
					<c:if test="${!empty noticeList }">
						<c:forEach items="${noticeList }" var="notice">
							<tr style='font-size: 0.85em; cursor: pointer;'
								onclick="OpenWindow('detail.do?from=list&nno=${notice.nno}','공지상세',700,800);">
								<td>${notice.nno }</td>
								<td id="boardTitle"
