...
public class JDBCTest {
	 ...
	 * 		(JDBC API버전 4이상에서는 getConnection()메서드에서 자동으로 
	 * 		 로딩해주기 때문에 생략할 수 있다.그렇지만 생략하지 않고 사용할 예정)
	 * 2. DB에 접속하기 ==> 접속이 완료되면 Connection객체가 반환된다.
	 * 		DriverManage.getConnection()메서드를 이용한다.
	 * 3. 질의 ==> SQL문장을 DB서버로 보내서 결과를 얻어온다
	 * 		(Statement객체 또는 PrepardStatement객체를 이용하여 작업한다.)
	 * 4. 결과 처리 ==> 질의 결과를 받아서 원하는 작업을 수행한다.
	 * 		1) SQL문의 select문일 경우
	 * 			==> select된 결과가 ResultSet객체에 저장되어 반환한다.
	 * 		2) SQL문의 select문이 아닐 경우(insert,update,delete)
	 * 			==> 정수값 즉, 해당 명령어 실행에 성공한 레코드 수를 반환한다.
	 * 5. 사용했던 자원 반납하기 ==> close()메소드 사용
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(rs.next()) {
				...
				//형식 2) rs.get자료형이름(컬럼번호) ==> 컬럼번호는 1번부터 시작
