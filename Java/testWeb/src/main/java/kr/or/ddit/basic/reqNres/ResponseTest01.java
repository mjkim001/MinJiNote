...
public class ResponseTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		/*
		 * - forward 방식 ==> 특정 서블릿에 대한 요청을 다른 서블릿이나 JSP로
		 * 					 제어를 넘긴다.(파라미터도 넘길 수 있다.)
		 * 				 ==> 클라이언트의 웹브라우저의 URL주소가 변경되지 않는다.
		 * 					 (처음 요청할 때의 URL주소로 유지된다.)
		 * 				 ==> 서버 내부에서만 접근이 가능하다.
		 * 				 ==> redirect보다 성능이 좋다.
		 * 예) 현재 서블릿에서 responseForwardTest.do주소로
		 * 	   forward방식으로 이동하기
		 * 
		 ...
		 * 			 기술하면 된다."
		 * 			 예) 이동할 주소가 "http://localost/testWeb/test.do"라고
		 * 				 하면 컨텍스트패스 이후의 주소인 "/test.do"만 기술하면 된다.
		 * 
		 */
		//request.getParameter("userName");
		
		// 이용되는 페이지로 현재 서블릿에서 구성한 데이터를 넘기려면
		// request객체의 setAttribute()메소드를 이용해서 넘겨준다.
		// 형식) setAttribute("키값",넘겨줄 데이터)메서드를 가지고 온다.
