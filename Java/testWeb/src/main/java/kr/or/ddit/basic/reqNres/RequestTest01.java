...
public class RequestTest01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//POST방식으로 전달되는 메이터의 문자 인코딩 방식 설정
		...
		// getParameterValues()메서드를 이용해서 'hobby'를 
		// 설정한 form의 체크박스 값들을 모두 가져오기
		...
		out.println("<h2> Request 객체 제공 메서드</h2>");
		out.println("<ul>");
		...
		//		이 메소드엣 반환하는 Map객체는 key값은 파라미터명이고
		//		자료형은 String이다.
		...
		for(String paramName : paramMap.keySet()) {
			...
			if(paramValue==null ||paramValue.length==0) {
				continue;
			}else if(paramValue.length==1) { //파라미터가 배열이 아니면
